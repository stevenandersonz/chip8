...
func (r *registers) OrVxVy(vx uint8, vy uint8) {
     r.GP[vx] = r.GP[vx] | r.GP[vy]
}
func (r* registers) AndVxVy(vx uint8, vy uint8)  {
    r.GP[vx] = r.GP[vx] & r.GP[vy]
}
func (r* registers) XOrVxVy (vx uint8, vy uint8)  {
     r.GP[vx] = r.GP[vx] ^  r.GP[vy]
}
func (r* registers) AddVyVx (vy uint8, vx uint8)  {
...
func (r *registers) ShiftRVx (vx uint8) {
    if r.GP[vx] && 0x01 == 1 {
        r.GP[15] = 1
    } else {
        r.GP[15] = 0
...
func (r *registers) SubNVxVx (vx uint8, vy uint8) {
    if vy > vx {
        r.GP[15] = 1
    } else {
        r.GP[15] = 0
...
func (r *registers) ShiftLVx (vx uint8) {
    if r.GP[vx] && 0x1 {
        r.GP[15] = 1
    } else {
        r.GP[15] = 0
